...
class GuardianAccess():
    ...
    def search(self, session, term, section="world", production_office="US", from_date="2020-10-01", page=1):
    ...
    def search_and_download(self, session, term):
        ...
        search_response = self.search(session, term, from_date="2020-10-01", production_office=production_office)
        print(json.dumps(search_response, indent='\t'))
        for page in range(search_response["response"]["pages"]):
            page=page + 1
            search_response = self.search(session, term, from_date="2020-10-01", production_office=production_office, page=page)
            #print(json.dumps(search_response, indent='\t'))
